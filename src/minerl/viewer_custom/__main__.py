...
def main(opts):
    ...
    # load rewards
    csv_path_rewards = os.path.join(data.data_dir, opts.stream_name, 'rewards_0.0.csv')

    rewards = load_csv(csv_path_rewards)

    # load goal state using opts.goal_state
    goal_state_pov=opts.goal_state

    # load coords
    csv_path_coords = os.path.join(data.data_dir, opts.stream_name, 'coords_0.0.csv')
